...
7.  选择自动调优的模式**Monitoring**或**Active**。
    ...
    **说明：** 推荐的调优策略为：
    ...
    -   **Slot-Utilization-Based**
        ...
        -   参数说明
            ...
            |slot-utilization-based.scale-down.window-size.min|统计`slot-utilization`的窗口大小。|30，单位为分钟。|计算`slot-utilization`的默认时间窗口为30分钟。|
    ...
    -   **Memory-Utilization-Based**
        ...
        -   参数说明
            ...
            |memory-utilization-based.memory-scale-up.max|单个Task Manager的内存调整时能调整到的最大值。|16Gi|Task Manager内存调整上限为16Gi。**说明：** VVP 2.2.2版本不支持基于GC的调优。 |
...
10. 填写**自定义配置**。
    ...
    **说明：** 以上参数冒号（:）号和参数值之间必须有1个空格，没有空格会无法保存该配置。
