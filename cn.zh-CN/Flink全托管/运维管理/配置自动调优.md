    ...
    **说明：** 推荐的调优策略为：
    ...
    -   **CPU-Based**（默认调优策略）
        ...
        -   使用限制
            ...
            -   不能同时开启**CPU-Based**和**Slot-Utilization-Based**策略。
        -   参数说明
            ...
            |cpu-based.scale-up.window-size.min|Scale Up时，CPU使用率的统计时间窗口大小。|1，单位为分钟。|1分钟计算一次CPU使用率，如果该1分钟内CPU使用率超过80%，则触发Scale Up操作。|
            |cpu-based.scale-down.window-size.min|Scale Down时，CPU使用率的统计时间窗口大小。|30，单位为分钟。|30分钟计算一次CPU使用率，如果30分钟内CPU使用率低于20%，则触发Scale Down操作。|
    ...
    -   **Source-Delay-Based**
        ...
        **说明：** 如果您的数据源为SLS或DataHub，使用**CPU-Based**策略后，调优效果不佳，可以使用Source-Delay-Based策略进行动态Scale Up。
        ...
        -   参数说明
            ...
            |source-delay-based.scale-up.window-size.min|延迟变化率统计周期窗口大小。|3，单位为分钟。|每3分钟统计一次作业延迟和`slot-utilization`值，如果达到阈值，则触发Scale Up操作。|
    ...
    -   **Slot-Utilization-Based**
        ...
        -   使用限制
            ...
            -   该策略不能和**CPU-Based**策略同时开启。
        -   参数说明

            |参数|说明|默认值|默认值解释|
            |--|--|---|-----|
            |slot-utilization-based.threshold|触发Scale Down时的`slot-utilization`。|0.5|当`slot-utilization`低于50%时，则触发Scale Down操作。|
            |slot-utilization-based.scale-down.window-size.min|统计`slot-utilization`的窗口大小。|30，单位为分钟。|每30分钟统计一次`slot-utilization`的使用率。|
    ...
    -   **Memory-Utilization-Based**
        ...
        -   参数说明
            ...
            |参数|说明|默认值|默认值解释|
            |--|--|---|-----|
            |memory-uitlization-based.memory-usage-max.threshold|可以容忍的最大内存利用率|0.95|TaskManager的内存使用率超过95%时，则触发调大内存操作。|
            |memory-uitlization-based.memory-usage-min.threshold|可以容忍的最小内存利用率|0.3|TaskManager的内存使用率低于30%时，则触发调小内存操作。|
            |memory-uitlization-based..memory-usage.target-utilization|目标资源利用率|0.8|调整TaskManager内存时，目标内存使用率达到80%。|
            |memory-uitlization-based.scale-up.window-size.min|内存Scale Up的监控周期|3，单位为分钟。|计算过去3分钟内存使用率，如果该使用率超过95%，则触发调大内存操作。|
            |memory-uitlization-based.scale-down.window-size.min|内存Scale Down的监控周期|30，单位为分钟。|计算过去30分钟内存使用率，如果该使用率低于30%，则触发调小内存操作。|
            |memory-uitlization-based.gc-ratio.threshold|GC频率触发内存Scale Up阈值|0.1|每秒GC次数超过0.1时，触发调大内存操作。|
            ...
            |memory-uitlization-based.memory-scale-up.ratio|GC rate和time超过阈值时，调大内存的倍数。其中：            -   GC rate是指JVM运行时，平均每秒GC的次数。
