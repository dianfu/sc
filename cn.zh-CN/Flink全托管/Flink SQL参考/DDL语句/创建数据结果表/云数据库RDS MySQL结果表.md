...
-   Flink全托管写入RDS或DRDS数据库结果表原理：针对Flink全托管每行结果数据，拼接成一行SQL语句，输入至目标端数据库，然后执行。如果使用批量写，需要在URL后面加上参数`?rewriteBatchedStatements=true`，以提高系统性能。
-   RDS MySQL数据库支持自增主键。如果Flink全托管写入数据支持自增主键，则在DDL中不声明该自增字段即可。例如，ID是自增字段，Flink全托管DDL不声明该自增字段，则数据库在一行数据写入过程中会自动填补相关自增字段。
-   如果DRDS有分区表，拆分键必须在Flink全托管DDL里PRIMARY KEY（）中声明，否则拆分的表无法写入。
...
-   Q：Flink全托管的结果数据写入RDS表，是按主键更新的，还是生成1条新的记录？
...
-   Q：使用RDS表中的唯一索引进行GROUP BY时需要注意什么？
    ...
    -   RDS中只有一个自增主键，Flink全托管作业中不能声明为PRIMARY KEY。
